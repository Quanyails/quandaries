...
class SymmetricalPairs:
    ...
    def variations(pair):
        ...
        ctpair = SymmetricalPair(
            first=pair.second.as_transposed(),
            second=pair.first.as_transposed(),
            symmetry=pair.symmetry,
        )
        return [
            pair,
...
def _find_intersecting_pairs_inner(trunk1: str, trunk2: str, branch1: str, branch2: str) -> List[SymmetricalPair]:
    ...
    for i1, i2 in intersecting_pairs:
        ...
        vars = SymmetricalPairs.variations(pair)

        # Only keep one variation of each symmetrical pair
        if not set(vars) & symmetrical_pairs:
            symmetrical_pairs.add(pair)

    return list(symmetrical_pairs)


def find_trunk_pairs(trunks: List[str]) -> List[SymmetricalPair]:
    assert len(trunks) >= 2, f"Must have at least two trunks, instead received {len(trunks)}"

    all_symmetrical_pairs: Set[SymmetricalPair] = set()

    for trunk1, trunk2 in combinations(trunks, 2):
        symmetrical_pairs = _find_intersecting_pairs_inner(trunk1, trunk1, trunk2, trunk2)
        all_symmetrical_pairs.update(symmetrical_pairs)

    return list(all_symmetrical_pairs)


def find_trunk_branch_pairs(trunks: List[str], branches: List[str]) -> List[SymmetricalPair]:
...
def _find_next_degree_pair(pair1: SymmetricalPair, pair2: SymmetricalPair) -> SymmetricalPair | None:
    ...
    # Merges did not remain symmetrical
    if merged1.degree != merged2.degree:
        return None
...
def find_next_degree_pairs(symmetrical_pairs: List[SymmetricalPair]) -> List[SymmetricalPair]:
    ...
    for pair1, pair2 in combinations(symmetrical_pairs, 2):
        var1 = SymmetricalPairs.variations(pair1)
        var2 = SymmetricalPairs.variations(pair2)
        for parvar1, parvar2 in product(var1, var2):
            next_degree_pair = _find_next_degree_pair(parvar1, parvar2)
            if next_degree_pair:
                varnext = SymmetricalPairs.variations(next_degree_pair)
                if not set(varnext) & next_degree_pairs:
